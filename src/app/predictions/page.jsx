        ...
        <div className="bg-[#1A1E24] rounded-[23px] overflow-hidden border border-red-500/10 h-full">
          ...
          <div className="bg-gradient-to-r from-red-900/50 to-rose-900/50 p-6 sm:p-8 flex items-center justify-between border-b border-red-500/20">
            <div className="flex items-center gap-4">
              ...
              <div>
                <h3 className="text-xl sm:text-2xl font-black text-white italic tracking-tighter">야구토토 스페셜 트리플 <span className="text-red-400">AI 마킹 베스트</span></h3>
                <p className="text-sm text-red-200/80 mt-1">예상 스코어 기반 확률업 64조합 (각 팀 2개 대역 마킹)</p>
              </div>
            </div>
            <div className="hidden sm:flex flex-col text-right">
              <span className="text-xs text-red-300 mb-1">권장 64조합 소액금액</span>
          ...
          <div className="bg-slate-900/80 p-4 text-center border-t border-white/5 sm:hidden">
             <span className="text-xs text-slate-400 mb-1 block">권장 64조합 소액금액</span>
             <span className="text-xl font-black text-white">6,400<span className="text-xs font-bold text-slate-500 ml-1">원</span></span>
          </div>
    ...
    <div className="container mx-auto px-4 py-12 max-w-6xl">
      ...
      {gamesWithPicks.length === 0 ? (
        ...
        <>
          ...
          {/* Toto Special Triple Section (Dynamic mapping based on DB toto_game_no) */}
          <TotoSpecialTriple targetGames={gamesWithPicks
            .filter(g => g.toto_game_no)
            .sort((a, b) => a.toto_game_no - b.toto_game_no)
          } />
        </>
      )}
